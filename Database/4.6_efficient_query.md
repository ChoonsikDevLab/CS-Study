...
```
Inner Join은 두 테이블에서 일치하는 값이 있는 행만 반환하지만, Left Outer Join은 왼쪽 테이블의 모든 행과 오른쪽 테이블에서 일치하는 행만을 반환합니다. 일치하는 값이 없다면 오른쪽 테이블의 열은 Null 값으로 채워집니다.
```
...
뷰는 여러 테이블을 조인 하거나 복잡한 조건을 포함한 쿼리를 하나의 뷰로 만들어 반복적인 쿼리 작성을 줄이거나, 특정 사용자에게 테이블의 특정 데이터만을 보여주고자 할 때도 사용됩니다.
```
...
- Sort Merge Join
Outer, Inner Table을 정렬한 다음 NL Join과 같이 중첩 for문들 순회하며 Join하는 방식입니다. 정렬된 테이블들은 메모리 영역에 저장되기에 테이블 랜덤 액세스가 발생하지 않아 빠릅니다. Inner Table에 마땅한 인덱스가 없거나 대량의 데이터를 Hash Join으로 처리할 수 없을 때 사용합니다.
- Hash Join
...
MySQL 5.5 이하에서 서브쿼리는 Full Scan을 수행하여 성능이 좋지 않고, 5.5 이상에서도 복잡한 서브쿼리의 경우 최적화가 충분하지 않을 수 있어 Join을 이용하는 것이 좋습니다.
```
